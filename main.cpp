...
bool init_mpu();
bool init_gprs();
bool send_gsm_command(const String &cmd, const char* expected, unsigned long timeout);
bool send_gsm_http_post(const String &payload);
bool wait_for_gsm_response(const char* expected, unsigned long timeout);
void init_gps();
...
bool wait_for_gsm_response(const char* expected, unsigned long timeout) {
  ...
  Serial.println(String("[GSM] Timeout waiting for: ") + expected);
  return false;
}
...
bool send_gsm_command(const String &cmd, const char* expected, unsigned long timeout) {
  GSM.println(cmd);
  return wait_for_gsm_response(expected, timeout);
}
...
bool send_gsm_http_post(const String &payload) {
  ...
  if (!send_gsm_command("AT+HTTPINIT", "OK", 5000)) return false;
  if (!send_gsm_command(String("AT+HTTPPARA=\"CID\",1"), "OK", 5000)) return false;
  if (!send_gsm_command(String("AT+HTTPPARA=\"URL\",\"") + BACKEND_URL + "\"", "OK", 5000)) return false;
  if (!send_gsm_command("AT+HTTPPARA=\"CONTENT\",\"application/json\"", "OK", 5000)) return false;

  GSM.print("AT+HTTPDATA=");
  GSM.print(payload.length());
  GSM.println(",10000");
  if (!wait_for_gsm_response("DOWNLOAD", 5000)) {
    Serial.println("[GPRS] HTTPDATA download prompt failed");
    send_gsm_command("AT+HTTPTERM", "OK", 5000);
    return false;
  }

  GSM.print(payload);
  if (!wait_for_gsm_response("OK", 10000)) {
    Serial.println("[GPRS] HTTPDATA send failed");
    send_gsm_command("AT+HTTPTERM", "OK", 5000);
    return false;
  }

  if (!send_gsm_command("AT+HTTPACTION=1", "+HTTPACTION:", GSM_HTTP_TIMEOUT)) {
    Serial.println("[GPRS] HTTPACTION failed");
    send_gsm_command("AT+HTTPTERM", "OK", 5000);
    return false;
  }

  delay(1000);
  send_gsm_command("AT+HTTPREAD", "OK", 10000);
  send_gsm_command("AT+HTTPTERM", "OK", 5000);
  return true;
}
...
void wait_for_network() {
  ...
  const int NETWORK_CHECK_RETRIES = 6;
...
bool connect_gprs() {
  update_lcd_status("GPRS", "Starting...");
  Serial.println("[GPRS] Starting GPRS connection...");

  if (!send_gsm_command("AT+SAPBR=3,1,\"CONTYPE\",\"GPRS\"", "OK", 5000)) return false;
  if (!send_gsm_command(String("AT+SAPBR=3,1,\"APN\",\"") + GSM_APN + "\"", "OK", 5000)) return false;
  if (String(GSM_APN_USER).length() > 0) {
    if (!send_gsm_command(String("AT+SAPBR=3,1,\"USER\",\"") + GSM_APN_USER + "\"", "OK", 5000)) return false;
  }
  if (String(GSM_APN_PASS).length() > 0) {
    if (!send_gsm_command(String("AT+SAPBR=3,1,\"PWD\",\"") + GSM_APN_PASS + "\"", "OK", 5000)) return false;
  }
  if (!send_gsm_command("AT+SAPBR=1,1", "OK", 20000)) return false;
  if (!send_gsm_command("AT+SAPBR=2,1", "+SAPBR:", 5000)) return false;

  gprsReady = true;
  update_lcd_status("GPRS Ready", "Connected");
  Serial.println("[GPRS] GPRS connected successfully");
  return true;
}
...
void setup() {
  ...
  digitalWrite(MOTOR_ENA, LOW);   // Motors off initially
  digitalWrite(MOTOR_ENB, LOW);   // Motors off initially
  ...
  // Setup PWM for both motors (on ENA/ENB for L293D)
  ...
  ledcAttachPin(MOTOR_ENA, MOTOR_PWM_CHANNEL_1);
  ledcAttachPin(MOTOR_ENB, MOTOR_PWM_CHANNEL_2);
  ...
  if (gprsReady) {
    Serial.println("[Backend] GPRS initialized");
  }
...
void start_motor() {
  ...
  // Set forward direction for Motor 1 (IN1=HIGH, IN2=LOW)
  digitalWrite(MOTOR_IN1, HIGH);
  digitalWrite(MOTOR_IN2, LOW);
  
  // Set forward direction for Motor 2 (IN3=HIGH, IN4=LOW)
  digitalWrite(MOTOR_IN3, HIGH);
  digitalWrite(MOTOR_IN4, LOW);
  
  // Apply PWM to ENA/ENB for speed control
  ledcWrite(MOTOR_PWM_CHANNEL_1, 255);  // Motor 1 full speed
  ledcWrite(MOTOR_PWM_CHANNEL_2, 255);  // Motor 2 full speed
  digitalWrite(MOTOR_ENA, HIGH);         // Enable Motor 1
  digitalWrite(MOTOR_ENB, HIGH);         // Enable Motor 2
  
  // Debug output
  Serial.println("[MOTOR] PWM via ENA(18): 255, ENB(25): 255");
...
void stop_motor() {
  // Stop both motors via L293D enable pins
  digitalWrite(MOTOR_ENA, LOW);   // Disable Motor 1
  digitalWrite(MOTOR_ENB, LOW);   // Disable Motor 2
...
void send_to_backend() {
  if (!gprsReady) {
    Serial.println("[GPRS] Skipping backend send, GPRS not ready");
    return;
  }
  ...
  jsonDoc["device_id"] = "SIM800L-device";
  ...
  if (send_gsm_http_post(jsonString)) {
    Serial.println("[GPRS] Backend data sent successfully");
  } else {
    Serial.println("[GPRS] Backend data send failed");
  }
}
