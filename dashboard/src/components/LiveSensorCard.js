...
export default function LiveSensorCard() {
  ...
  if (!data) {
    return (
      ...
      >
        <Card sx={{ mb: 2 }}>
          <CardContent>Loading live sensor data...</CardContent>
        </Card>
      </motion.div>
    );
  }
  ...
  return (
    ...
    >
      <Card sx={{ mb: 2, boxShadow: 3, borderRadius: 2 }}>
        {/* Status Bar */}
        <Box sx={{ height: 8, backgroundColor: data && isEngineRunning(data.lcd_display) ? 'success.main' : 'error.main', borderRadius: '4px 4px 0 0' }} />
        <CardContent>
